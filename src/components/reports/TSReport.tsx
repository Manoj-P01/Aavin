...
export default function TSReport({ rows, totals, date }: Props) {
  const rowsBySection = SECTIONS.reduce((acc, s) => {
    acc[s] = rows.filter(r => r.section === s);
    return acc;
  }, {} as Record<string, TSMilkRow[]>);
          ...
          <tbody>
            {SECTIONS.map(section => {
              ...
              return (
                <>
                  {/* Section header */}
                  <tr key={`hdr-${section}`} className="section-header">
                    <td colSpan={10} style={{
                  ...
                  {/* Section total */}
                  <tr key={`tot-${section}`} className="row-total">
